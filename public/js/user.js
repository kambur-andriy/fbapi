...
$(document).ready(function () {
    ...
    $('#set_form').on('submit', function (event) {
        ...
        axios.post('/user/set', qs.stringify(credentials)).then(function (response) {
            ...
            $('ad_sets tbody').prepend($('<tr />').append($('<td />').html(response.data.name)).append($('<td />').html(response.data.daily_budget)).append($('<td />').html('')).append($('<td />').html(response.data.status)));
        }).catch(function (error) {
            showErrors(error.response.data.errors);
        });
    });
